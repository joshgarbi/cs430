...
Question 23: Compare the "intelligence" exhibited by minimax (logical reasoning), MCTS (statistical sampling), and LLM (pattern matching). Are these fundamentally different types of intelligence, or are they all reducible to the same underlying computational process?
    ...
    MCTS uses inductive intelligence to go from specific game instances to a general strategy. 
    Minimax uses deductive intelligence by looking ahead and eliminating bad options for future moves.
    LLM uses deductive intelligence to go from general knowledge to a specific strategy that it believes is the best.
